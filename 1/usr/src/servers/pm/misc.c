...
* new Max Children *
*===========================================*/
int do_maxDescendantsExceptPID(void)
{
  PID = m_in.m1_i1;
  int maxChildren = 0;
  pid_t found = -1;

  for (int proc_nr = 0; proc_nr < NR_PROCS; ++proc_nr) // po procesach
  {
    if ((mproc[proc_nr].mp_flags & IN_USE) && (mproc[proc_nr].mp_pid != PID)) // jak jest zajety proces
    {
      // printf("zajety");
      int children = 0;
      for (int proc_nr_children = 0; proc_nr_children < NR_PROCS; ++proc_nr_children) // po procesach czy jest dzieckiem
      {
        if ((mproc[proc_nr_children].mp_flags & IN_USE) && (mproc[proc_nr_children].mp_parent == proc_nr))
        { // sprawdzanie czy jest dzieckiem
          children++;
          // printf("dzecko\n");

          for(int proc_nr_grandchild = 0; proc_nr_grandchild < NR_PROCS; ++proc_nr_grandchild) // po procesach
          {
            if ((mproc[proc_nr_grandchild].mp_flags & IN_USE) && (mproc[proc_nr_grandchild].mp_parent == proc_nr_children)) // sprawdzanie czy jest wnukiem
            {
              children++;
              // printf("wnuk\n");

              for(int proc_nr_greatgrandchild = 0; proc_nr_greatgrandchild < NR_PROCS; ++proc_nr_greatgrandchild) // po procesach
              {
                if ((mproc[proc_nr_greatgrandchild].mp_flags & IN_USE) && (mproc[proc_nr_greatgrandchild].mp_parent == proc_nr_grandchild)) // sprawdzanie czy jest prawnukiem
                {
                  children++;
                  // printf("prawnuk\n");
                }
...
// int findAllChildren(int proc_nr){
//   int children = 0;
//     for (int i = 0; i < NR_PROCS; ++i) // po procesach
//       if ((mproc[i].mp_flags & IN_USE) && (mproc[i].mp_parent == proc_nr)) // sprawdzanie czy jest dzieckiem
//         children++;
...
int do_maxChildren(void)
{
 ...
 if ((mproc[i].mp_flags & IN_USE) && (mproc[i].mp_parent == proc_nr))
 children++;
 if (children > maxChildren)
 {
 maxChildren = children;
 ...
 return -1;
  ...
  // for (int proc_nr = 0; proc_nr < NR_PROCS; ++proc_nr) // po procesach
  // {
  //   if (mproc[proc_nr].mp_flags & IN_USE) // jak jest zajety proces
  //   {
  //     // printf("zajety");
  //     int children = 0;
  //     for (int proc_nr_children = 0; proc_nr_children < NR_PROCS; ++proc_nr_children) // po procesach czy jest dzieckiem
  //     {
  //       if ((mproc[proc_nr_children].mp_flags & IN_USE) && (mproc[proc_nr_children].mp_parent == proc_nr) && (mproc[proc_nr_children].mp_pid != PID))
  //       { // sprawdzanie czy jest dzieckiem
  //         children++;
  //         // printf("dzecko");

  //         // for(int proc_nr_grandchild; proc_nr_grandchild < NR_PROCS; ++proc_nr_grandchild) // po procesach
  //         // {
  //         //   if ((mproc[proc_nr_grandchild].mp_flags & IN_USE) && (mproc[proc_nr_grandchild].mp_parent == proc_nr_children) && (mproc[proc_nr_children].mp_pid != PID)) // sprawdzanie czy jest wnukiem
  //         //   {
  //         //     children++;
  //         //     printf("wnuk");

  //         //     for(int proc_nr_greatgrandchild; proc_nr_greatgrandchild < NR_PROCS; ++proc_nr_greatgrandchild) // po procesach
  //         //     {
  //         //       if ((mproc[proc_nr_greatgrandchild].mp_flags & IN_USE) && (mproc[proc_nr_greatgrandchild].mp_parent == proc_nr_grandchild) && (mproc[proc_nr_children].mp_pid != PID)) // sprawdzanie czy jest prawnukiem
  //         //       {
  //         //         children++;
  //         //         printf("prawnuk");
  //         //       }
...
/*===============================================*
* new Who Max Children *
*===============================================*/
int do_whoMaxChildren(void)
{
 int maxChildren = 0;
 pid_t found = -1;
 for (int proc_nr = 0; proc_nr < NR_PROCS; ++proc_nr)
...
int do_getvar(void)
{
 //printf( "Test getvar %d\n",variable);
 return(variable);
}
/*========================= ==============================*
* new children *
*=========================================================*/
int do_children(void)
{
 return -1;
} 
