...
int curr_p_group = 2;				/* so first group will be A */
 ...
 * possibly promoting another user to head of the queue from the given group.
 */
 register int i;
 register int next_p_group;
 register struct proc *tmp;
 register struct proc *found;

  if (rdy_head[USER_Q] == NIL_PROC) return;		/* if empty do nothing */
  
  for(i = 1; i <=3; i++)				/* iterate through p_groups */
  {
  	next_p_group = (curr_p_group + i) % 3;		/* calculate next p_group */
  	tmp = rdy_head[USER_Q];
  	while((tmp != NIL_PROC) && (tmp->p_group != next_p_group))	/* while next process is not in desired group */
  	{
  		tmp = tmp->p_nextready;
  	}
  	if (tmp->p_group = next_p_group)		/* sanity check */
  	{
  		curr_p_group = next_p_group;
  		break;
  	}
  }
  
  if (tmp != rdy_head[USER_Q])				/* if the process is not in the head */
  {
  	found = tmp;
  	tmp = rdy_head[USER_Q];			/* tmp is head */
  	
  	while(tmp->p_nextready != found)		/* find where hole is */
  	{
  		tmp = tmp->p_nextready;
  	}
  	
  	tmp->p_nextready = found->p_nextready;		/* connect list */
  	found->p_nextready = rdy_head[USER_Q];		/* point to second element */
  	rdy_head[USER_Q] = found;			/* move process to head */
  	
  	if (found == rdy_tail[USER_Q])
  	{
  		rdy_tail[USER_Q] = tmp;		/* if found was last tail is moved accordingly */
  	}
