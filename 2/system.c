...
FORWARD _PROTOTYPE( int do_get_group, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_set_group, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_get_quant, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_set_quant, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
	    ...
	    case SYS_GET_GROUP: r = do_get_group(&m);	break;
	    case SYS_SET_GROUP: r = do_set_group(&m);	break;
      	    case SYS_GET_QUANT: r = do_get_quant(&m); break;
      	    case SYS_SET_QUANT: r = do_set_quant(&m); break;
 ...
 *				do_get_group				     *
 *===========================================================================*/
PRIVATE int do_get_group(m_ptr)
register message *m_ptr;	/* pointer to request message */
{
  struct proc* p = proc_addr(m_ptr->m1_i1);
  return isuserp(p) ? (int)p->p_group : ESRCH;  /* ESRCH - no such process */
}

/*===========================================================================*
 *				do_set_group				     *
 *===========================================================================*/
PRIVATE int do_set_group(m_ptr)
register message *m_ptr;	/* pointer to request message */
{
  register int new_group = m_ptr->m1_i2;
  struct proc* p = proc_addr(m_ptr->m1_i1);

  /* Check if the call is valid - changes the prority of a user process
     and changes it to a known value */
  if (!isuserp(p)) return ESRCH;
  if (new_group != PGRP_A && new_group != PGRP_B && new_group != PGRP_C) return EINVAL;

  p->p_group = (unsigned char)new_group;
  return new_group;
}
/*===========================================================================*
 *				do_get_quant		     *
 *===========================================================================*/
PRIVATE int do_get_quant(m_ptr)
register message *m_ptr;	/* pointer to request message */
{
  struct proc* p = proc_addr(m_ptr->m1_i1);
  return isuserp(p) ? QuantTime[(int)p->p_group] : ESRCH;  /* ESRCH - no such group */
}
/*===========================================================================*
 *				do_set_quant				     *
 *===========================================================================*/
PRIVATE int do_set_quant(m_ptr)
register message *m_ptr;	/* pointer to request message */
{
  register int group = m_ptr->m1_i1;
  register int new_time = m_ptr->m1_i2;

  if (group != PGRP_A && group != PGRP_B && group != PGRP_C) return EINVAL;

  QuantTime[group] = (int)new_time;
  return new_time;
}
...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  rpc->p_group = PGRP_A;	/* by default process is in gropu A*/
